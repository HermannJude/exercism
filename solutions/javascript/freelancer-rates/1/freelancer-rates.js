...
export function dayRate(ratePerHour) {
  ...
  return ratePerHour * workedHoursPerDay
}
...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const fullMonthPrice = numMonths * billableDaysPerMonth * (dayRate(ratePerHour) * (1-discount))
  const remainingDaysPrice = remainingDays * dayRate(ratePerHour)
  
  return Math.ceil(fullMonthPrice + remainingDaysPrice);
}
